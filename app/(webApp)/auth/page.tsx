...
import { FaYoutube, FaLinkedin, FaTwitter, FaFacebook, FaInstagram, FaReddit, FaDiscord, FaGithub } from "react-icons/fa";
...
function Page() {
    ...
    try {
      ...
      if (verifyResponse.data.success) {
        if (verifyResponse.data.data && verifyResponse.data.access_token) {
          ...
          cookieSetter(verifyResponse.data.data, verifyResponse.data.access_token);
        } else {
          ...
          if (loginResponse.data.success) {
            cookieSetter(loginResponse.data.data, loginResponse.data.access_token);
          } else {
      ...
      if (error.response?.status === 404) {
        try {
          ...
          if (fallbackResponse.data.success) {
            ...
            setShowOtpVerification(true);
            startOtpTimer(); // Start 10-minute countdown
            toast.success(fallbackResponse.data.message || "OTP sent to your email!");
          }
        } catch (fallbackError: any) {
          const errorMessage = fallbackError.response?.data?.message || "Failed to send OTP. Please try again.";
          toast.error(errorMessage);
        }
      } else {
  ...
  const resendOtp = async () => {
    ...
    try {
      setLoading(true);
      const response = await axios.post(`${API}/resend-otp`, { email: email.toLowerCase() });
      if (response.data.success) {
        setOtp("");
        startOtpTimer(); // Restart 10-minute countdown
        toast.success(response.data.message || "OTP has been resent to your email.");
      } else {
    ...
    try {
      ...
      if (isGoogleSignup && googleCredential) {
        ...
        if (response.data.success) {
          ...
          toast.error(response.data.message || "Google sign up failed. Please try again.");
        }
        return;
      }
      ...
      if (response.data.success) {
        ...
        const userData = response.data.data || {
          profilePicUrl: response.data.user?.profilePicUrl ? `${API}${response.data.user.profilePicUrl}` : "",
      ...
      if (error.response?.data?.message?.includes("already exists") || 
        ...
        try {
          ...
          if (verifyResponse.data.success && verifyResponse.data.data && verifyResponse.data.access_token) {
            cookieSetter(verifyResponse.data.data, verifyResponse.data.access_token);
            return;
          }
        } catch (verifyError) {
          // If verification fails, show error
          toast.error("Account exists but OTP verification failed. Please try logging in.");
          router.push("../auth");
        }
      } else {
  ...
  useEffect(() => {
    ...
    const initializeGoogleSignIn = () => {
      if (!window.google || !googleButtonRef.current || !process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID) return;
    ...
    const initializeGoogleSignUp = () => {
      if (!window.google || !googleButtonRefSignUp.current || !process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID) return;
      ...
      try {
        ...
        window.google.accounts.id.renderButton(googleButtonRefSignUp.current, {
          theme: "outline",
          size: "large",
          text: "signup_with",
          width: "100%",
        ...
        {signup ? (
          <div className="w-sm p-8">
            ...
            {sign === 1 && (
              <>
                <div className="flex flex-col justify-center  items-center mb-4">
                  ...
                  {isGoogleSignup && (
                    <p className="text-xs text-green-600 mt-2">✓ Google account connected</p>
                  )}
                </div>
               
                {!isGoogleSignup && (
                  <>
                    <p className="mb-2 text-sm text-gray-700 font-semibold text-center w-full">Sign up with</p>
                    <div className="w-full mb-4">
                      <div ref={googleButtonRefSignUp} className="w-full"></div>
                    </div>
                ...
                <div className="flex border rounded-xl px-3 py-2 gap-2 mb-4">
                  <span className="mr-2 text-black"><CgRename /></span>
                  <input
                    type="text"
                    value={fullname}
                ...
                <div className="flex border rounded-xl px-3 py-2 gap-2 mb-4">
                  <span className="mr-2 text-black"><MdDriveFileRenameOutline /></span>
                  <input
                    type="text"
                    value={username}
                ...
                {isGoogleSignup && (
                  <p className="mb-2 text-sm text-gray-700">Enter phone number (optional)</p>
                )}
                {isGoogleSignup && (
                  <div className="flex border rounded-xl px-3 py-2 gap-2 mb-4">
                    <span className="mr-2 text-black"><LuPhone /></span>
                    <input
                      value={phone}
            ...
            {sign === 2 && !isGoogleSignup && !showOtpVerification && (
              <>
                ...
                <div className="flex border rounded-xl px-3 py-2 gap-2 mb-4">
                  <span className="mr-2 text-black"><LuMails /></span>
                  <input
                    value={email}
            ...
            {sign === 2 && showOtpVerification && (
              <>
                ...
                <p className="text-sm mb-2 text-center">Enter the 6-digit OTP sent to</p>
                <p className="text-sm mb-2 text-center font-semibold text-gray-700">{email}</p>
                {otpTimer > 0 ? (
                  <p className="text-xs mb-4 text-center text-gray-600">
                    OTP valid for: <span className="font-semibold text-black">{formatTimer(otpTimer)}</span>
                  </p>
                ) : (
                ...
                <div className="flex border rounded-xl px-3 py-2 gap-2 mb-4">
                  <span className="mr-2 text-black"><HiOutlineMail /></span>
                  <input
                    value={otp}
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={verifyOtp}
                    disabled={loading || otp.length !== 6 || otpVerified || otpTimer === 0}
                    className="bg-black text-white w-full py-2 text-center block rounded-xl text-sm font-medium mb-3 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {loading ? "Verifying..." : otpVerified ? "Verified" : otpTimer === 0 ? "OTP Expired" : "Verify OTP"}
                  </button>
                </div>
                <button
                  onClick={resendOtp}
            ...
            {sign === 3 && (
              <>
                ...
                <p className="text-sm mb-4 text-center">Where did you hear about Relmes?</p>
                <div className="grid grid-cols-2 gap-3 mb-4">
                  <button
                    onClick={() => setReferralSource("youtube")}
                    className={`flex flex-col items-center justify-center p-4 border-2 rounded-xl transition-all ${
                      referralSource === "youtube" ? "border-black bg-gray-50" : "border-gray-200"
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`flex flex-col items-center justify-center p-4 border-2 rounded-xl transition-all ${
                      referralSource === "linkedin" ? "border-black bg-gray-50" : "border-gray-200"
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`flex flex-col items-center justify-center p-4 border-2 rounded-xl transition-all ${
                      referralSource === "twitter" ? "border-black bg-gray-50" : "border-gray-200"
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`flex flex-col items-center justify-center p-4 border-2 rounded-xl transition-all ${
                      referralSource === "facebook" ? "border-black bg-gray-50" : "border-gray-200"
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`flex flex-col items-center justify-center p-4 border-2 rounded-xl transition-all ${
                      referralSource === "instagram" ? "border-black bg-gray-50" : "border-gray-200"
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`flex flex-col items-center justify-center p-4 border-2 rounded-xl transition-all ${
                      referralSource === "reddit" ? "border-black bg-gray-50" : "border-gray-200"
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`flex flex-col items-center justify-center p-4 border-2 rounded-xl transition-all ${
                      referralSource === "discord" ? "border-black bg-gray-50" : "border-gray-200"
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`flex flex-col items-center justify-center p-4 border-2 rounded-xl transition-all ${
                      referralSource === "other" ? "border-black bg-gray-50" : "border-gray-200"
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`flex flex-col items-center justify-center p-4 border-2 rounded-xl transition-all col-span-2 ${
                      referralSource === "referred" ? "border-black bg-gray-50" : "border-gray-200"
                    }`}
                  >
          ...
          <div className="w-sm p-8">   
            ...
            <div className="flex justify-between items-center mb-4">
                ...
                <p className="mb-2 text-sm text-gray-700 font-semibold">Sign in with</p>
            ...
            <p className="mb-2 text-sm text-gray-700 font-semibold self-center">Or</p>
              ...
              <div className="flex border rounded-xl px-3 py-2 gap-2">
                ...
                <button
                  ...
                  className="bg-black text-white w-full py-2 text-center block rounded-xl text-sm font-medium mb-3 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {loading ? "Sending OTP..." : otpSent ? "OTP Sent" : "Continue"}
                </button>
              </>
            ) : (
              <>
                <div className="flex flex-col justify-center items-center mb-4">
                  ...
                  <p className="text-sm text-gray-600 mt-2">Enter the OTP sent to {email}</p>
                  {otpTimer > 0 ? (
            ...
            <button
              ...
              className="bg-black text-white w-full py-2 text-center block rounded-xl text-sm font-medium mb-3 disabled:opacity-50 disabled:cursor-not-allowed"
            >
                    {loading ? "Verifying..." : otpTimer === 0 ? "OTP Expired" : "Verify & Sign In"}
            </button>
          </div>
                <button
                  onClick={async () => {
